...
의존성:
    python -m pip install pyhwp5 olefile lxml
"""
...
def _ensure_hwp2hwpx():
    ...
    # GitHub에서 사용자 캐시로 클론. skill 디렉토리 내부에 untracked 파일을 만들지 않는다.
    cache_root = os.environ.get("LOCALAPPDATA") or os.path.expanduser("~/.cache")
    clone_dir = os.path.join(cache_root, "codex", "hwp2hwpx-python-refactor")
...
def main():
    parser = argparse.ArgumentParser(
        description="HWP(바이너리) to HWPX(개방형 XML) 변환"
    )
