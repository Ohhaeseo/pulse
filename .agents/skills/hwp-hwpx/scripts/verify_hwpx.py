...
def _print_report(report):
    """검수 결과를 콘솔에 출력한다."""
    print(f"\n{'='*60}")
    print(f"  HWPX 검수 결과: {report['status']}")
    print(f"{'='*60}")
    ...
    if "comparison" in report:
        ...
        for key in ["paragraphs", "runs", "tables", "images", "bindata"]:
            if key in c:
                ...
                marker = "OK" if d["diff"] == 0 else ("WARN" if d["diff"] > 0 else "FAIL")
                print(f"  [{marker}] {key}: {d['source']} -> {d['result']} ({diff_str})")
        if "section_size_ratio" in c:
            ratio = c["section_size_ratio"]
            marker = "OK" if ratio >= 90 else ("WARN" if ratio >= 50 else "FAIL")
            print(f"  [{marker}] section 크기 비율: {ratio}%")
